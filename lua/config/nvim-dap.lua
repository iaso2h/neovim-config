...
M.config = function()
    ...
    map("n", [[dG]],        [[:lua require('dap').goto_()<cr>]], {"silent"})

    map("n", [[dr]], [[:lua require('dap').repl.open()<cr>]], {"silent"})
    map("n", [[db]], [[:lua require('dap').toggle_breakpoint()<cr>]], {"silent"})
    map("n", [[dB]], [[:lua require('dap').list_breakpoints()cr>]],   {"silent"})
    ...
    fn.sign_define('DapBreakpoint', {text='●',  texthl='Error',      linehl='', numhl='Error'})
